...
def test_ffdnet(**args):
    ...
    input_path = "input1.jpg"  # Changed default input to 'input.jpg'
    if not os.path.exists(input_path):
        raise FileNotFoundError(f"Could not find the input image at {input_path}")

    rgb_den = is_rgb(input_path)

    # Open image as a CxHxW torch.Tensor
    if rgb_den:
        in_ch = 3
        model_fn = 'models/net_rgb.pth'
        imorig = cv2.imread(input_path)
        ...
        imorig = cv2.imread(input_path, cv2.IMREAD_GRAYSCALE)
    ...
    if not args['dont_save_results']:
        ...
        cv2.imwrite("noisy.jpg", noisyimg)
        cv2.imwrite("ffdnet.jpg", outimg)
        if args['add_noise']:
            cv2.imwrite("noisy_diff.jpg", variable_to_cv2_image(diffnoise))
            cv2.imwrite("ffdnet_diff.jpg", variable_to_cv2_image(diffout))
